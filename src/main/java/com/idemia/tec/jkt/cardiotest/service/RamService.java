...
//        }
        ...
        if (ram.isUseSpecificKeyset())
            ramBuffer.append(ramCase1(ram.getCipheringKeyset(), ram.getAuthKeyset(), ram.getMinimumSecurityLevel()));
        else {
            for (SCP80Keyset keyset : root.getRunSettings().getScp80Keysets()) {
                ramBuffer.append("\n; using keyset: " + keyset.getKeysetName() + "\n");
                ramBuffer.append(ramCase1(keyset, keyset, ram.getMinimumSecurityLevel()));
            }
        }
        ramBuffer.append("\n.UNDEFINE %EF_CONTENT\n");
...
//        }
    ...
    private String ramCase1(SCP80Keyset cipherKeyset, SCP80Keyset authKeyset, MinimumSecurityLevel msl) {
    ...
    private String ramCase1NegativeTest(SCP80Keyset cipherKeyset, SCP80Keyset authKeyset, MinimumSecurityLevel msl) {
        StringBuilder routine = new StringBuilder();
        routine.append(
                "\n.POWER_ON\n"
                        + proactiveInitialization()
        ...
        routine.append(
                "\n; command(s) sent via OTA\n"
//                        + appendScriptSelect3g(root.getRunSettings().getRam().getCustomTargetEfBadCase())
                        + ".SET_BUFFER J 00 D6 00 00 <?> AA ; update binary\n"
                        + ".APPEND_SCRIPT J\n"
                        + ".END_MESSAGE G J\n"
                        + "; show OTA message details\n"
